...
const CreateNewListingForm = () => {
  ...
  const onFinish = async (value) => {
    ...
    if (value.title) {
      const res = await postNewListing(value.title, address, value.price, imageUrl, metaData);
      if (res.status) {
        console.log(res);
  ...
  return (
    <>
      ...
      >
        ...
        <Form.Item style={{ display: "flex", justifyContent: "center", flexDirection: "row" }}>
          <Button type="primary" size="large" htmlType="submit" onClick={onFinish}>
            Submit
          </Button>
        </Form.Item>
